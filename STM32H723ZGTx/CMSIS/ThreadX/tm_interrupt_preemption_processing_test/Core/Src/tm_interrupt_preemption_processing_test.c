...
void  tm_interrupt_preemption_processing_initialize(void)
{
    ...
    tm_thread_create(0, 10, tm_interrupt_preemption_thread_0_entry);

    /* Create thread that generates the interrupt at priority 10.  */
    tm_thread_create(1, 13, tm_interrupt_preemption_thread_1_entry);
    ...
    tm_thread_create(5, osPriorityRealtime, tm_interrupt_preemption_thread_report);
    tm_thread_resume(5);
}
