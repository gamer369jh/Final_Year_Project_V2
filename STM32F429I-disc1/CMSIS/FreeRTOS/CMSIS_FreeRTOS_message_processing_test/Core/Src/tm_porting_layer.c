...
{
	   /* osStatus_t status = osMessageQueuePut(tm_queue_array[queue_id], message_ptr, 0U, 0U);
	    if (status != osOK) {
	        return TM_ERROR;
	    }
...
int  tm_queue_receive(int queue_id, unsigned long *message_ptr)
{
	    ...
	    /*osStatus_t status = osMessageQueueGet(tm_queue_array[queue_id], message_ptr, 0U, osWaitForever);
	   	    if (status != osOK) {
	   	        return TM_ERROR;
	   	    }
